    ...
    const struct lwan_key_value *additional_headers)
{
    ...
    if (request->flags & RESPONSE_CHUNKED_ENCODING) {
        ...
        /* If there's no Content-Length, then there's no Content-Type */
...
skip_content_type:
    if ((status < HTTP_BAD_REQUEST && additional_headers)) {
        ...
        for (header = additional_headers; header->key; header++) {
            ...
            case MULTICHAR_CONSTANT_L('C', 'o', 'n', 'n'):
                if (LIKELY(streq(header->key + 4, "ection")))
    ...
    if (LIKELY(!connection_overridden)) {
...
next_frame:
    ...
    return (request->flags & REQUEST_IS_WEBSOCKET) == REQUEST_IS_WEBSOCKET;
}
